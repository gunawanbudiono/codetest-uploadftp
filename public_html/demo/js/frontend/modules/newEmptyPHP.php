var Events  =   {
        ...
        fireEvent   :function(){
            ...
            $.each($('.js-btn-remove-multidate'),function(i,e){
                $(e).off().on('click',function(){
                    ...
                    $('.js-form-custom-date').eq(i).remove();
                    

                    //update array multidate
            ...
            $.each($('.js-btn-delete-multidate-item'),function(i,e){
                    ...
                    Component.create.confirm.action(
                        {
                            ...
                            $('.multidate-items').eq(i).remove();
                            $('.js-btn-multidate-items').eq(i).remove();

                            if($('.js-btn-multidate-items').length > 0){
                                
                                $this.multidate.data=[];       
                                
                                $.each($('.js-btn-multidate-items'),function(a,b){
                                    $this.multidate.data.push({
                                        'i_event'       :   $this.selectedId,
                                        'start_date'    :   $('.js-btn-multidate-items').eq(a).data('startdate'),
                                        'start_hour'    :   $('.js-btn-multidate-items').eq(a).data('starthour'),
                                        'start_minute'  :   $('.js-btn-multidate-items').eq(a).data('startminute'),
                                        'start_time'    :   $('.js-btn-multidate-items').eq(a).data('starttime'),
                                        'end_hour'      :   $('.js-btn-multidate-items').eq(a).data('endhour'),
                                        'end_minute'    :   $('.js-btn-multidate-items').eq(a).data('endminute'),
                                        'end_time'      :   $('.js-btn-multidate-items').eq(a).data('endtime'),
                                    }); 
                                                                        
                                    
                                    $this.multidate.data.sort(Global.sortArrayObjectItems("start_date"));
                            ...
                            console.table($this.multidate.data);

                            Component.create.jbox.notif('Ahaa..','Data sudah dihapus!',true);
    ...
    tickets   :   {
        ...
        setProperty :   function(a){
            ...
            if(a.scope=='free'){
                ...
                if(Events.tickets.scope=='edit'){
                    ...
                    $('.js-txt-ticket-earlybird-value').val(a.discount).number(true,0);                    
                ...
                Component.create.confirm.action(
                    {                                        
                        ...
                        if(Events.tickets.mode=='create'){
                            Request.do('POST',Request.Tickets.set(),function(){
        ...
        handler :   function(){
            ...
            $('#btn-cal-disc').click(function(){
                ...
                $('.js-txt-ticket-discount-fix').val($('.js-txt-ticket-discount-price').val()-(parseFloat(a)*parseFloat(b))/100).number(true,0);
                return false;
            });
            ...
            $('#btn-cal-disc-early').click(function(){
               ...
               $('.js-txt-ticket-early-fix').val(($('.js-txt-ticket-earlybird-price').val()-(parseFloat(a)*parseFloat(b))/100)).number(true,0);
    ...
    onEdit  :   {
        ...
        backdrop    :function(panels){
            ...
            if(panels.length > 0){
                ...
                if(panels[0]!='box-cover-events'){
                    
                    $('.panel').find($('.form-focus')).removeClass('form-focus');
        ...
        checkStepComplete    :   function(data){
            ...
            if(data.events.length > 0){
                ...
                $.each(data.events,function(i,e){
                    ...
                    if(e.start_date != null){
                        ...
                        $('.js-txt-event-publish-date').val(e.start_date);
                    }
            ...
            if(data.materi.content.length > 0){                                                                
                ...
                if(data.materi.content[0].items['materi'].length ==1){
            ...
            console.log("Venue: "+completeLokasi);
            ...
            if(completeBanner &&
                ...
                if($('.tooltip8').length > 0){
                    ...
                    //Component.moveDomToCenter('.tooltip8');
            ...
            if(completeBanner &&
                ...
                if(Events.is_published==0){
                    ...
                    //Component.moveDomToCenter('.tickets-button');
                }
        ...
        default : function(){
            ...
            Request.do('GET',Request.Events.get.byId(Global.b64EncodeUnicode($this.selectedId),'json'),function(){
                ...
                $this.onEdit.checkStepComplete($this.data);
                                
                $this.categories = Request.response.data.categories;
                $this.renderPanel(Request.response.data);
                
                me.setDefaultValue(Request.response.data.events[0]);                                
        ...
        handler :function(){
            ...
            $('.js-btn-save-draft').off().on('click',function(){
                var a = "Apa yang terjadi bila saya simpan sebagai draft<br>";
                a+="Event / kelas ini tidak akan muncul di halaman home dan tidak akan terlihat oleh Publik.<br>";
            ...
            $.each($('.js-action-venue-date'),function(i,e){
                $(e).off().on('click',function(){
                    ...
                    if($this.is_multiday==1){ //multi session
                        ...
                        console.log('type : multi session');
                        $('.js-form-table-custom-date').empty();                    
                        //render form
                        $.each(Events.data.multiday,function(i,e){                            
                            $('.js-form-table-custom-date').append(Render.events.form.multidate());
                            $('.js-txt-event-custom-start-date').eq(i).val(e.start_date);
                        });
                        
                        $('.js-txt-event-custom-start-date').first().val($(e).data('start'));
                    ...
                    Component.create.confirm.action(
                        {
                            ...
                            Global.param.fields = {
                                ...
                                end_minute      : '00',
                                end_time        : 'AM',
                                is_multiday     :   Global.b64EncodeUnicode($this.is_multiday),
                                days_count      : 1
                            };
            ...
            $('.js-submit-custom-date').off().on('click',function(){
                ...
                $('#js-form-custom-date').parsley('validate');

                if($('#js-form-custom-date').parsley('isValid')){
            ...
            $('.js-btn-save-ticket-free').off().on('click',function(){
                ...
                var scope = Events.tickets.scope;
                var ticket_name = $('.js-txt-ticket-type').val();                    
                var price_type = $('.js-opt-ticket-price-type').find(":selected").val();
            ...
            $('.js-btn-save-ticket-earlybird').off().on('click',function(){                
                    ...
                    Component.create.confirm.action(
                        {                                        
                            ...
                            Global.param.fields = {
                                i_event             :   $this.code,
                                ticket_name         :   $('.js-txt-ticket-type').val(),
                                ticket_description  :   $('.js-txt-ticket-description-earlybird').val(),
                                ...
                                ticket_available    :   $('.js-txt-ticket-earlybird-seats').val(),
                                ticket_paid         :   0,
                                ticket_retur        :   0, 
                                is_free             :   $this.tickets.isFree,
                                is_promo            :   $this.tickets.isPromo,
                            ...
                            Global.param.earlybird = $this.tickets.earlybird.data;

                            //commit to db
                            Request.do('POST',Request.Tickets.setEarlyBird(),function(){
                ...
                console.log($this.tickets.earlybird.data); 
            ...
            $('.js-btn-publish-now').off().on('click',function(){
                ...
                if($('#js-btn-publish-now').parsley('isValid')){
                    ...
                    if($this.start_date > publish_date ){
                        ...
                        Component.create.jbox.notif('Opps...','Tanggal yang dipilih tidak boleh lebih dari tanngal mulai event!',false);
                    }
        ...
        cekMultidateOnPush  :function(val){
            ...
            if(val == null){r=true};
            if(val < $this.multidate.lastDate){r=false;err='Tanggal tidak boleh lebih kecil dari tanggal terskhir'};
            if(val == $this.multidate.lastDate){r=false;err='Tanggal tidak boleh lebih sama dengan tanggal terskhir'};                                    
        ...
        setDefaultValue :function(data){
            ...
            var $this = this;
            var ev = new Events.const(data);
                        
            $('.js-txt-events-title').val(ev.event_name);
            $('.js-txt-events-subtitle').val(ev.event_subname);
            $('#js-txt-event-tag').val(ev.event_tag);
                                                
            Global.setEditorEventValue(ev.event_description);
    ...
    renderPanel :   function(data){
        ...
        $this.onEdit.checkStepComplete(data);
        ...
        if(data.events.length >0){ //events
            ...
            $.each(data.events,function(i,e){ //single record no worries in loop coy
                ...
                if(e.event_lat==null){
                    ...
                    //Component.create.jbox.tooltip('Opps..',Render.displayTooltip('Lokasi dan Alamat belum di set.'),false,'.panel');
                    //$('.panel').eq(1).trigger('click');
                ...
                if(e.event_lat!=null && e.event_lat!=null ){
                    ...
                    if(e.is_multiday==0){ //single session
                        ...
                        table+="<table class='table'>";
                            ...
                            table+="<tbody>";
                                ...
                                //loop 
                                $.each(data.multiday,function(x,y){ //single record no worries in loop coy
